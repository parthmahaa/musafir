...
function wishlist() {
  ...
  const getWishlist = async() =>{
    try{
        const response= await fetch(`http://localhost:5000/wishlist?userEmail=${Email}`,{
          method : "GET"
        })
        if(response.ok){
                ...
                <div className="flex-grow p-3">
                  <div className='flex justify-between items-center'>
                    <div>
                      ...
                      <a href={item.Location} target='_blank' rel="noopener noreferrer" className="ml-0 text-blue-500 text-sm">
                          Location
                        </a>
                    </div>
                    <button onClick={() => handleDelete(item.Name)} className="ml-2">
